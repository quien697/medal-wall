...
## Purpose
Own how app-authored text reaches the screen in the user's chosen language: a single String
Catalog as the source of every UI string, one stored preference resolved consistently for both
SwiftUI-rendered text and display strings built outside the view hierarchy, and a firm boundary
that leaves user-entered content exactly as it was typed.
## Requirements

# distance-units Specification

## Purpose
Own how a distance value becomes text on screen: which unit the app displays in and how the
device seeds that choice, the rule that storage stays kilometre-canonical whatever the user
picked, the conversion and rounding applied for display, and the decision that preset
distances are named rather than measured. Pace belongs here too, since it is a distance

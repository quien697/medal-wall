...
- Replace the `AsyncImage` `.empty` (loading) branch in `RaceImage`, `MedalImage`, and
  `AvatarImage` with a shimmer skeleton shaped and sized to that component's final image
  region (rounded-rect for race, inner circle for medal and avatar).
...
- **Edited:** `RaceImage.swift`, `MedalImage.swift`, `AvatarImage.swift` — `.empty` branch
  plus a named "loading" `#Preview` in each.
